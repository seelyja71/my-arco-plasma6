...
var emptyGeneralResponse = { redirectUrl: 'data:text;charset=utf-8,', requestHeaders: [] };
...
const privacyUrl = "https://www.standsapp.org/privacy-policy/";
const termsUrl = 'https://www.standsapp.org/end-user-license-agreement/';
var lastActivity;
...
const MEASUREMENT_ID = 'G-NX73VCVX7Q';
const API_SECRET = '5muC8QF0T7yUQoFOLauMsA';
...
function onBeforeNavigate(details) {
	...
	frameDatas[details.tabId] = frameDatas[details.tabId] || {};
...
function parseExtras(extras) {
	...
	for(var i of  args ){
		if(i.includes('=')) {
			let [paramName, paramValue] = i.split('=')
...
function isEasylistBlock(details, topHostAddress) {
	if (topHostAddress?.includes('yahoo.com') &&
		[
			...
			'https://players.brightcove.net/videojs-ima3/5.1.1/videojs-ima3.min.js',
		].includes(details.url)) {
		return false
	}

	var {url,initiator, type } = details
	//url = getUrlHost(url)
	initiator = getUrlHost(initiator)
	var cur = newUrl.generic.blacklist
	var splitted_url = url.split(/\/(.*)/, 2).filter((el)=> !!el)
	var host = getUrlHost(url).split('.')
	if (getUrlHost(url).includes('youtube.com')){
		return false
	}
	for(var i = 0; i< host.length; i++){
		cur = cur && cur[host[i]]
	}
	if(cur === undefined || cur.path === undefined){
		return false;
	}
	if (cur.path === '*' || ( new URL(url).pathname.includes(cur.path))){
		let result = true;
		if (!cur.args) {
			var extras = cur.extras
			if (extras) {
				if ('third-party' in extras) {
					// If the third-party option is specified, 
					// the filter is only applied to requests 
					// from a different origin than the currently viewed page.
					if (initiator.includes(host[0])) {
						result = false;
					} else {
						result = true;
					}					
				...
				if ('domain' in extras) {
					...
					if (domain){
						if((initiator in domain) && (domain[initiator] == true)){
							result = true
						}
						else{
							result = false;// TODO: SKip domain with false more clever
						}
					}
					if (type in extras) {
						result = true
...
function onBeforeRequest(details) {
	try {
		if (details.url.includes('c.evidon')){ //Solution for sites like crunchyroll.
															//Evidon is solution for Cookie consent. It's disables on web request API cause header Non-Authoritative-Reason: WebRequest API
			return
		}
		...
		if (isEasylistBlock(details, pageData?.topHostAddress)) {
			blockingResponse.cancel = true;
			blockingResponse.block = true;
		}

		let pathname = new URL(details.url).pathname
		if (pathname.endsWith('/ad.js') || (pathname.endsWith('/ads.js'))){
			blockingResponse.cancel = true
			blockingResponse.block=true
		}
		...
		if (blockingResponse.block) {
			...
			if (blockingResponse.typeId == stndz.blockTypes.selfRedirect){// redirecting to somewhere, where is 200/204/201 etc. response type will be retuned.
				/*
				TODO: - redirecting to http and redirecting to another host can sometimes cause net::ERR_UNSAFE_REDIRECT error.
				 nowadays I fixes this by redirecting to `origProto://origHost/`, but in my mind maybe there is a  better solution.
				*/

				var	 new_url = new URL(details.url)
				new_url = new_url.protocol + new_url.host.replace('www.', '')
				return emptyGeneralResponse
				return {redirectUrl: new_url} // this is not `breaks` request. instead it just gives you empy response.

				/*TODO: very important, that I can't redirect to `data:` link, but
				   https://developer.chrome.com/docs/extensions/reference/webRequest/#type-BlockingResponse says the oposite. In my mynd
				   maybe, if fix this, we can remove type selfRedirect, and this block of code at all
				* */
			...
			if (details.type == "main_frame" && !(exclude_block.includes(details.initiator))) {
...
function onCommited(details) {
	if (details.frameId == 0 && pageDatas[details.tabId]) {
		...
		if (testWindows[details.tabId]) {
			...
			// Todo: check that we can really delete this line from here
			// dispatchEvent(new CustomEvent(stndz.customEvents.navigation, { detail: pageDatas[details.tabId] }));
		}
	}
}
...
function setPageReferrer(pageData, referrer) {
	try {
		...
		} else if (pageData.previousUrl && pageData.previousUrl.indexOf(referrer) === 0) {
			pageData.referrer = pageData.previousUrl
		} else if (pageData.openerUrl && pageData.openerUrl.indexOf(referrer) === 0) {
...
function onTabActivated(details) {
	if (testWindows[details.tabId]) {
		removeWindow(testWindows[details.tabId]);
		return;
	}
	...
	callIfTabExists(details.tabId, async (currentTab) =>{
		const pageData = pageDatas[details.tabId];
		if (pageData && pageData.isValidSite && !pageData.hasStands) {
			const manifest = chrome.runtime.getManifest();
			const contentScripts = manifest.content_scripts?.filter((scripts) =>
				scripts.js ? scripts.js.length > 1 : false
			) || [{ js: [''], matches: [''] }];

			for (const file of contentScripts[0].js) {
				try {
					await executeFileOnTab(details.tabId, file);
				}	catch (e) {}
			}

			const callWhenAddedToExistingPage = (machineId, appInstallPageHint) => {
			...
			try {
				switch (pageData.hostAddress) {
					case 'facebook.com':
						await executeFileOnTab(details.tabId, "content/sites/fb.js", false,);
						break;
					case 'youtube.com':
					case 'youtu.be':
						await executeFileOnTab(details.tabId, "content/sites/yt.js", false);
						break;
					case 'twitch.tv':
						await executeFileOnTab(details.tabId, "content/sites/twitch.js", false);
						break;
					case 'instagram.com':
						await executeFileOnTab(details.tabId, "content/sites/ig.js", false);
						break;
					default:
						break;
				}
...
function onTabUpdated(tabId, changeInfo, tab) {
	if(changeInfo && changeInfo.status === 'loading' && changeInfo.url !== undefined && pageDatas[tabId]) {
		if( previousTabUrl !== changeInfo.url){
			if(pageDatas[tabId].pageUrl.startsWith('https://www.youtube.com')
				&& changeInfo.url.startsWith('https://www.youtube.com')
			){
				pageDatas[tabId].referrer = pageDatas[tabId].pageUrl
			}
		}
	}
	if (changeInfo.url && changeInfo.url !== previousTabUrl && pageDatas[tabId]!== undefined ) {
		previousTabUrl = changeInfo.url
		pageDatas[tabId].pageUrl = changeInfo.url
		pageDatas[tabId].loadTime = utcTimeGetter()
		dispatchEvent(new CustomEvent(stndz.customEvents.navigation, { detail: pageDatas[tabId] }));
	}
...
function removeTabIfExists(tabId, isClosing) {
	...
	if (pageData) {
		...
		if (timeOnPage >= 2000) {
			...
			if (getRandomWithinRange(1, 100) <= 2 && pageData.isValidSite && pageData.pageUrl != newTabUrl && pageData.previousHost != 'newtab' && pageData.host != 'about:blank' && !testWindows[tabId]) {
				sendMessageToBackground({
...
function reportUtm() {
    $st.onUserReady(function (userData) {
        chrome.tabs.query({}, (data) => {
			urls = data.map((value) => value.url);
			serverLogger.log(stndz.logEventTypes.reportUtm, {
...
function onInstalled(details) {
	if (details.reason == 'install') {
		...
		// Report all open tabs on install
		runOnPageDataLoaded = function () {
			loadAnonyReport(function (report) {
				reporter.reportOpenTabs(report.id)
			});
		}
		...
		serverLogger.log(stndz.logEventTypes.extensionInstalled, {
		...
		runOnAllTabs(function(tab) {
			if (tab.id !== undefined && (
				tab.url?.includes('detail/stands-adblocker') ||
				tab.url?.includes('detail/adblocker-stands') ||
				tab.url?.includes('addon/stands-fair-adblocker') ||
				tab.url?.includes('addons/detail/fair-adblocker')
			)) {
			...
			if (tab.url.indexOf(getBrowserStoreUrl()) > -1) {
				isStoreInstall = true;
				...
				if (tab.active) {
					...
					isStoreDetailPageInstall = tab.url.indexOf(getBrowserStoreUrl() + "detail") > -1 && tab.url.indexOf(extensionId) > -1;
				} else if (tab.url.indexOf(getBrowserStoreUrl() + "detail") > -1 && tab.url.indexOf(extensionId) > -1) {
					storeTitle = tab.title;
				} else if (!storeTitle) {
					storeTitle = tab.title;
				}
			...
			checkAppExists(function(exists) {
				runOnActiveTab(function(tab) {
					var url = `https://www.standsapp.org/thank-you-chrome${search}`;
...
function onMessage(request, sender, callback) {
	// console.log('getting message in background', request)
	try {
		switch (request.type) {
			...
			case stndz.messages.pageData:
				if (sender.tab && pageDatas[sender.tab.id]) {
					if (sender.frameId == 0) {
						setPageReferrer(pageDatas[sender.tab.id], request.referrer);
					}
			...
			case stndz.messages.getDashboardData:
				const getDashboardData = function(data) {
					const responseData = {
						forStandsPopup: true,
				...
				getDashboardData(dashboardStorage)
			...
			case stndz.messages.getBlockingData:
				$stats.runWhenStarted(function() {
					...
					const getBlockingDataCallback = function(data) {
						const responseData = {
							forStandsPopup: true,
			...
			case stndz.messages.getUserData:
				$st.onUserReady(function(userData) {
					...
					const getUserDataCallback = function(userData) {
						const responseData = {
							forStandsPopup: true,
					...
					getUserDataCallback(userDataCopy)
			...
			case stndz.messages.getUserSettings:
				const getUserSettingsCallback = function(settings) {
					const responseData = {
						forStandsPopup: true,
				...
				getUserSettingsCallback(stndz.settings)
			...
			case stndz.messages.updateUserSettings:
				...
				var updateData = { settings: request.settings };
						...
						chrome.tabs.query({}, (tabs) => {
							tabs.forEach((tab) => {
								...
								for (let i=0; i<request.hosts.length; i++) {
									if (url === request.hosts[i].hostAddress) {
										reloadTab(tab.id);
										break
									}
								}
			...
			case stndz.messages.getAdBlocker:
				checkHasAdBlocker(function() {
					const getAdBlockerCallback = function(adBlockerData) {
						const responseData = {
							forStandsPopup: true,
							type: stndz.messages.getAdBlocker + '-response',
							adBlockerData: adBlockerData
						}
						chrome.runtime.sendMessage(responseData)
					}
					getAdBlockerCallback({
						exists: hasAdBlocker
					})
					// callback && callback({
					// 	exists: hasAdBlocker
			...
			case stndz.messages.disableAdBlockers:
				var disableAdBlockers = function(managementPermissionsExisted) {
					adBlockerDetector.disable(function() {
						...
						const disableAdBlockersCallback = function(disabled) {
							const responseData = {
								forStandsPopup: true,
						...
						disableAdBlockersCallback(true)
						// callback && callback(true);
					});
				};
			...
			case stndz.messages.undoBlockedElements:
				...
				unblockElementsOnPage(activeTabId, "Dashboard", undoBlockedElementsCallback);
...
function updateIcon(tabId) {
	$st.onUserReady(function(userData) {
		...
		setAppIcon(pageDatas[activeTabId].isDeactivated || stndz.settings.enabled == false);
...
function updateIconBadge(tabId, notificationCount) {
	notificationCount = 0; //We don't need to show notifications bage to user
	if (stndz.settings.enabled == false) {
		setAppIconBadgeText(chrome.i18n.getMessage('off'));
		setAppIconBadgeTitle(chrome.i18n.getMessage('stands_is_paused'));
		return;
	}
	...
	$stats.runWhenStarted(function() {
		var badgeCounter = stndz.settings.iconBadgePeriod == stndz.iconBadgePeriods.Disabled ?
			"" :
			stndz.settings.iconBadgePeriod == stndz.iconBadgePeriods.Today ?
				$stats.getBlocksToday() :
				(pageData ? pageData.blocks : 0);
		var badgeTitle = stndz.settings.iconBadgePeriod == stndz.iconBadgePeriods.Disabled ?
			"" :
			stndz.settings.iconBadgePeriod == stndz.iconBadgePeriods.Today ?
			badgeCounter + ' ' + chrome.i18n.getMessage('blocks_today') :
			badgeCounter + ' ' + chrome.i18n.getMessage('blocks_on_this_page');
		var badgeText = badgeCounter > 0 ? badgeCounter.toLocaleString() : "";
		badgeTitle = notificationCount > 0 ?
			chrome.i18n.getMessage('you_have_unread_notifications', notificationCount) :
				badgeCounter > 0 ?
					badgeTitle :
					chrome.i18n.getMessage('stands');
...
function canFillDetectionSample(type, hostAddress, detectionHost) {
	...
	if (detectionSamplesQuotas[type][hostAddress][detectionHost] >= 2)
		return false;
...
function anonyReportForMalwareAnalysis(anonyId) {
	var ids = { nid: anonyId, pid: '', sid: '' };
	stndz.experiments.v2 && anonyReportForMalwareAnalysisV2(ids);
}

function anonyReportForMalwareAnalysisV2(ids) {
	reporter.startReporting(ids);
}
function createMalwareReporter() {
	var lastBulk = utcTimeGetter();
	var bulk = [];
	let ids = null;

	function reportBulk(force) {
		var now = utcTimeGetter();
		var timeoutPassed = (now - lastBulk) / 1000 >= stndz.experiments.max;
		var thresholdPassed = bulk.length >= stndz.experiments.bulk;
		if (bulk.length > 0 && (timeoutPassed || thresholdPassed || force)) {
			var reportBulk = Array.from(bulk);
			callIn(function() {
				reportForAnalysisV2(ids, reportBulk);
			}, 250);
	...
	return {
		startReporting(idsVal) {
			ids = idsVal;

			addEventListener(stndz.customEvents.navigation, function(e) {
				bulk.push(e.detail);
				reportBulk();
			});

			setInterval(reportBulk, 2000);
		},
		reportOpenTabs(anonyId) {
			ids = { nid: anonyId, pid: '', sid: '' };
...
const reporter = createMalwareReporter();
...
function reportForAnalysisV2(ids, bulk) {
	try {
		...
		var reportUrl = "https://analyze.standsapp.org/convert";
		var events = [];
		bulk.forEach(function(pageData) {
			var trq = pageData.trq && pageData.trq.length > 0 ? pageData.trq.join(',') : '';
			events.push({
				nid: ids.nid,
				pid: ids.pid,
				sid: ids.sid,
				cc: stndz.settings.geo,
				ts: pageData.loadTime.getTime(),
		...
		callUrl({ url: reportUrl, method: 'POST', data: { rows: events }, headers: [ { name: "Content-type", value: "application/json" } ] });
	} catch(e) { }
}
...
async function getOrCreateClientId() {
	return await new Promise((resolve) => {
		chrome.storage.local.get(
			'googleAnalyticsClientId', (result) => {
				let clientId = result.googleAnalyticsClientId;
				if (!clientId) {
					clientId = self.crypto.randomUUID();
					chrome.storage.local.set({googleAnalyticsClientId: clientId});
				}
				resolve(clientId)
			});
	})
...
async function reportGaInstalled() {
