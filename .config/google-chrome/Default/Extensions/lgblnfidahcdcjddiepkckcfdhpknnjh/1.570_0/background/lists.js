...
function reportAnonymousIdForDebug(anonymousId, isNew) {
    $st.onUserReady((userData) => {
        serverLogger.log(stndz.logEventTypes.reportAnonymousId, {
...
function loadAnonyReport(callback) {
    ...
    getStorageValue(anonyReportKey, function (success, value) {
        if (success && value) {
            reportAnonymousIdForDebug(value.id, false);
            callback(value);
        } else {
