...
function blockElementsFunc(blockCssValue, forceOpen, location) {
  ...
  if (currentDocument.documentElement.clientWidth *
    currentDocument.documentElement.clientHeight < minWindowSize)
    return
  ...
  function addViewAndControllerToPage() {
    ...
    helperWindowContainer.setAttribute('style',
      'all: initial; position: fixed; z-index: 2147483647; top: 0px; left: 0px; border: none; padding: 0px; margin: 0px; width: 100%; display: block !important;')
    currentDocument.documentElement.appendChild(helperWindowContainer)

    callUrl({
      url: getExtensionRelativeUrl(
        '/views/web_accessible/block-element/view.html'), raw: true,
    }, function (response) {
      response = response.replace(/{{path}}/g,
        getExtensionRelativeUrl('/views/web_accessible')).
        replace(/{{location}}/g, location == 'right' ? 'right' : 'left')
      helperWindowContainer.innerHTML = response

      helperWindow = currentDocument.getElementById(
        'stndz-block-element-window')
      currentDocument.getElementById('stndz-block-element-close').
        addEventListener('click', function (event) {
          event.preventDefault()
          exitBlockElement()
        }, true)
      currentDocument.getElementById('stndz-block-element-cancel').
        addEventListener('click', function (event) {
          event.preventDefault()
          exitBlockElement()
        }, true)
      currentDocument.getElementById('stndz-block-element-save').
        addEventListener('click', function (event) {
          event.preventDefault()
          saveBlockedElement()
        }, true)
      currentDocument.getElementById('stndz-block-element-undo').
        addEventListener('click', function (event) {
          event.preventDefault()
          undoSelectElement()
        }, true)
      currentDocument.getElementById('stndz-block-element-done').
        addEventListener('click', function (event) {
          event.preventDefault()
          exitBlockElement()
        }, true)

      var pickHeader = currentDocument.getElementById(
        'stndz-block-element-pick-header')
      pickHeader.addEventListener('mousedown', function (event) {
        event.preventDefault()
        startDragging(event.x, event.y)
      }, true)

      var chosenHeader = currentDocument.getElementById(
        'stndz-block-element-chosen-header')
      chosenHeader.addEventListener('mousedown', function (event) {
        event.preventDefault()
        startDragging(event.x, event.y)
      }, true)
  ...
  function onMouseMove(event) {
    if (isDragging()) {
      helperWindow.style.setProperty('top',
        (event.y + helperWindowMoving.topDiff) + 'px', 'important')
      helperWindow.style.setProperty('left',
        (event.x + helperWindowMoving.leftDiff) + 'px', 'important')
    }
    else if (window.allowSelectElement) {
      helperWindowContainer
      //currentDocument.documentElement.innerText = ''
      var elements = currentDocument.elementsFromPoint(
        event.x + currentDocument.documentElement.scrollLeft,
        event.y + currentDocument.documentElement.scrollTop)
      var target = elements.length == 0 ? null : (elements[0] == overlayElement
        ? elements[1]
        : elements[0])
      ...
      if (!target) {
        ...
        var isStandsAd = elementToOverlay.tagName == 'IFRAME' &&
          elementToOverlay.id &&
          elementToOverlay.id.indexOf(stndz.elements.iframeIdPrefix) == 0
        overlayElement.setAttribute('class', isStandsAd
          ? 'stndz-element-overlay-stands'
          : 'stndz-element-overlay-standard')

        var position = getElementPosition(elementToOverlay)
        updateOverlayPosition(position.x - 1, position.y - 1,
          elementToOverlay.clientWidth, elementToOverlay.clientHeight, true)
  ...
  function updateOverlayPosition(left, top, width, height, visible) {
    ...
    overlayElement.style.setProperty('display', visible ? 'block' : 'none',
      'important')
  }

  function selectElement(element) {
    var isStandsAd = element.tagName == 'IFRAME' && element.id &&
    ...
    while (topmostElement.tagName != 'BODY') {
      if (topmostElement.clientWidth == elementRect.width &&
        topmostElement.clientHeight == elementRect.height) {
        var topmostElementRect = topmostElement.getBoundingClientRect()
        if (topmostElementRect.left == elementRect.left &&
          topmostElementRect.top == elementRect.top) {
  ...
  function setViewActions() {
    var pickContainer = currentDocument.getElementById(
      'stndz-block-element-pick')
    var chosenContainer = currentDocument.getElementById(
      'stndz-block-element-chosen')
    var savedContainer = currentDocument.getElementById(
      'stndz-block-element-saved')
    var listContainer = currentDocument.getElementById(
      'stndz-block-element-list')
    ...
    window.applyElementSelectionOnView = function () {
      if (window.selectedElementsDetails.length > 0) {
        ...
        var addingElement = listContainer.children.length <
          window.selectedElementsDetails.length
        while (listContainer.firstChild) {
          listContainer.removeChild(listContainer.firstChild)
        }

        helperWindow.style.setProperty('height',
          (140 + (window.selectedElementsDetails.length * 20)) + 'px',
          'important');

        chosenContainer.style.setProperty('height',
          (140 + (window.selectedElementsDetails.length * 20)) + 'px',
          'important');
        ...
        for (var i = 0; i < window.selectedElementsDetails.length; i++) {
          ...
          if (addingElement && i + 1 == window.selectedElementsDetails.length) {
            item.style.setProperty('animation-name',
              'stndz-highlight-animation', 'important')
          ...
          item.addEventListener('mouseenter', function (event) {
            var code = (function () {
              ...
              replace('(i)', '(' + event.target.getAttribute('index') + ')')
            executeOnTab('(' + code + ')();')
          })
    ...
    window.applySavedOnView = function () {
      var doneSeconds = 5
      var doneButton = currentDocument.getElementById(
        'stndz-block-element-done')
  ...
  function isSelectableElement(element) {
    ...
    if (isStandsObject(element) || isStandsObject(element.parentElement) ||
      isStandsObject(element.parentElement.parentElement))
      return false
    ...
    var isDocumentSize = (element.offsetWidth >=
        currentDocument.documentElement.offsetWidth - 5 && element.offsetHeight >=
        currentDocument.documentElement.offsetHeight - 5) ||
      (element.offsetWidth >= currentDocument.documentElement.clientWidth - 5 &&
        element.offsetHeight >= currentDocument.documentElement.clientHeight -
        5)
  ...
  function getElementDetails(element) {
    ...
    while (details.cssSelector == '' || forceClimbToParent ||
    currentDocument.querySelectorAll(details.cssSelector).length != 1) {
      forceClimbToParent = false

      var elementSelector = getElementCssSelector(currentElement)
      var weakIdentification = elementSelector ==
      ...
      if (weakIdentification) {
        ...
        if (currentElement.parentElement.tagName == 'BODY') {
          if (currentElement.getAttribute('style'))
            elementSelector = currentElement.tagName.toLowerCase() +
      ...
      var newCssSelector = elementSelector +
        (details.cssSelector ? '>' + details.cssSelector : '')
      if (currentElement.parentElement.querySelectorAll(newCssSelector).length >
        1) {
        var sameTagElements = currentElement.parentElement.querySelectorAll(
          currentElement.tagName.toLowerCase())
        var directChildrenCounter = 0
        for (var i = 0; i < sameTagElements.length; i++) {
          if (sameTagElements[i].parentElement ==
            currentElement.parentElement) {
            directChildrenCounter++
            if (sameTagElements[i] == currentElement) {
              newCssSelector = currentElement.tagName.toLowerCase() +
                ':nth-of-type(' + directChildrenCounter + ')' +
                (details.cssSelector ? '>' + details.cssSelector : '')
              forceClimbToParent = true
              break
            }
    ...
    details.elementCount = currentDocument.querySelectorAll(
      details.cssSelector).length
    return details
  }
  ...
  function getElementPosition(elem) {
    ...
    rect = getOffset(elem)
    //alert(`${top}, ${left} +++ ${rect.top}, ${rect.left}`)
    return { x: rect.left, y: rect.top }
  }
  ...
  function animateOverlayElement(callback) {
    ...
    overlayElement.style.setProperty('top', overlayRect.top + 'px',
      'important')
    overlayElement.style.setProperty('left', overlayRect.left + 'px',
      'important')
    ...
    setTimeout(function () {
      ...
      overlayElement.style.setProperty('top',
        (helperRect.top + helperRect.height / 2) + 'px', 'important')
      overlayElement.style.setProperty('left',
        (helperRect.left + helperRect.width / 2) + 'px', 'important')
      overlayElement.style.setProperty('height', '0px', 'important')
      overlayElement.style.setProperty('width', '0px', 'important')
      overlayElement.style.setProperty('transition-duration', '500ms',
        'important')

      setTimeout(function () {
        updateOverlayPosition(0, 0, 0, 0, false)
        overlayElement.style.setProperty('position', 'absolute', 'important')
        overlayElement.style.setProperty('transition-duration', '200ms',
          'important')
        window.allowSelectElement = true
...
function unblockElementsFunc(cssRules) {
  ...
  for (var i in cssRules) {
    if (pageData.hostAddress == cssRules[i].host) {
      var currentRuleElementsCount = currentDocument.querySelectorAll(
        cssRules[i].cssSelector).length
...
function countBlockedElementsFunc(cssRules) {
  ...
  for (var i in cssRules) {
    if (pageData.hostAddress == cssRules[i].host) {
      elementsCount += currentDocument.querySelectorAll(
        cssRules[i].cssSelector).length
    }
  }
