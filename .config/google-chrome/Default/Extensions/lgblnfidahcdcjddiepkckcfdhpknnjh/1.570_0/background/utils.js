...
function getBrowserId() {
    if (!browserId) {
        if (navigator.userAgent.indexOf('Vivaldi') > -1) {
            browserId = browsers.Vivaldi;
        } else if (navigator.userAgent.indexOf('Edg/') > -1) {
            browserId = browsers.Edge;
        } else {
            browserId = browsers.Chrome;
        }
    }
...
function getBrowserName() {
    switch (getBrowserId()) {
        case browsers.Chrome:
            return browsers.Chrome.toString();
        case browsers.Vivaldi:
            return browsers.Vivaldi.toString();
        case browsers.Edge:
            return browsers.Edge.toString();
    }
}
