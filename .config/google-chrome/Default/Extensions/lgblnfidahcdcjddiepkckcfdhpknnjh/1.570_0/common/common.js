    ...
    try {
        ...
        if (obj.headers && obj.headers.length > 0) {
            for (var i in obj.headers)
                xhr.setRequestHeader(obj.headers[i].name, obj.headers[i].value);
        }

        xhr.send(obj.data ? typeof obj.data == "string" ? obj.data : JSON.stringify(obj.data) : null);
    } catch(e) {
